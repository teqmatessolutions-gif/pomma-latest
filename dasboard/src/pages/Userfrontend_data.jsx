...
export default function ResortCMS() {
    ...
    const sectionConfigs = {
        ...
        nearbyAttractions: { title: "Nearby Attraction", endpoint: "/frontend/nearby-attractions/", fields: [{ name: "title", placeholder: "Attraction Title" }, { name: "description", placeholder: "Description", type: "textarea" }, { name: "map_link", placeholder: "Google Maps Link" }, { name: "image", type: "file" }, { name: "is_active", type: "checkbox", placeholder: "Is Active?" }], isMultipart: true },
                        ...
                        {resortData.resortInfo.length > 0 ? resortData.resortInfo.map(item => (
                            <div key={item.id} className="bg-gray-50 border rounded-lg p-4 space-y-2 col-span-full">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-700 mt-2">
                                    ...
                                    {item.hotel_code && <p><strong>Aiosell Hotel Code:</strong> <span className="font-mono text-indigo-600">{item.hotel_code}</span></p>}
                                </div>
